...
import { TrzszError, type TrzszFileReader, type TrzszFileWriter } from "./comm";
...
export class BrowserFileReader implements TrzszFileReader {
  ...
  public async readFile(buf: ArrayBuffer) {
    ...
    try {
      const len = Math.min(buf.byteLength, this.getSize() - this.pos);
      const chunk = this.file?.slice(this.pos, this.pos + len);
      this.pos += len;
      return new Uint8Array(await chunk?.arrayBuffer() ?? new ArrayBuffer(0));
    } catch (err) {
      if (err instanceof Error && err.name === "NotReadableError") {
        throw new TrzszError(`No permission to read: ${this.relPath.join("/")}`);
      }
      throw new TrzszError(`Read ${this.relPath.join("/")} error: ${String(err)}`);
    }
  }
...
class BrowserFileWriter implements TrzszFileWriter {
  ...
  public closeFile() {
    if (!this.closed) {
      ...
      (this as Record<string, unknown>).writer = null;
      this.closed = true;
    }
  }
...
async function doCreateFile(handle: FileSystemDirectoryHandle, fullPath: string[]) {
  try {
    ...
    } else if (err instanceof Error && err.name === "TypeMismatchError") {
      throw new TrzszError(`Is a directory: ${fullPath.join("/")}`);
    }
    throw new TrzszError(`Write ${fullPath.join("/")} error: ${String(err)}`);
  }
}
...
async function doCreateDirectory(handle: FileSystemDirectoryHandle, fullPath: string[]) {
  try {
    ...
    throw new TrzszError(`Create ${fullPath.join("/")} error: ${String(err)}`);
  }
}
